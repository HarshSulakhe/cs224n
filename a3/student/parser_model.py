...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        embeddings = torch.index_select(self.embeddings, 0, w.view(-1))
        x = embeddings.view(-1, self.n_features * self.embed_size)
    ...
    def forward(self, w):
        ...
        h = torch.add(torch.matmul(x, self.embed_to_hidden_weight),
                      self.embed_to_hidden_bias)
        h = torch.nn.functional.relu(h)
        h = self.dropout(h)
        logits = torch.add(torch.matmul(
